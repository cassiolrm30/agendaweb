...
public class TarefasEdicaoController
{
	...
	public ModelAndView edicao(Integer id, HttpServletRequest request)
	{
		...
		try
		{
			...
			if (tarefa != null)
			{
				...
				model.setPrioridade(tarefa.getPrioridade() == 1 ? "ALTA" : tarefa.getPrioridade() == 2 ? "ALTA" : tarefa.getPrioridade() == 3 ? "BAIXA" : null);
	...
	@RequestMapping(value = "/alterar-tarefa", method = RequestMethod.POST)
	public ModelAndView cadastrarTarefa(TarefaEdicaoModel model, HttpServletRequest request)
	{
		ModelAndView modelAndView = new ModelAndView("tarefas-edicao");
		try {
